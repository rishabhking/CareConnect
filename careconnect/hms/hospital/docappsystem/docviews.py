...
def Patient_Appointment_Details_Remark(request):
    if request.method == 'POST':
        ...
        status = request.POST['status']
        patientaptdet = Appointment.objects.get(id=patient_id)
        # patient_new = User.objects.get(id=patientaptdet.pat_id_id)

        doctor_new = User.objects.get(id = request.user.id)

        if status == 'Approved':
            jitsi_meet_link = f"https://meet.jit.si/Appointment-{patient_id}-{patientaptdet.doctor_id.id}"
            patientaptdet.jitsi_link = jitsi_meet_link  
            patientaptdet.save()
            subject = "Your Appointment is Approved"
            message = f"""
            Dear {patientaptdet.pat_id.admin.first_name} ,
            
            Your appointment with Dr. {patientaptdet.doctor_id.admin.first_name} is approved.
            Please join the video consultation on {patientaptdet.date_of_appointment} at {patientaptdet.time_of_appointment} using the link below at the scheduled time:
            {jitsi_meet_link}
            ...
            message2 = f"""
            Dear Dr. {patientaptdet.doctor_id.admin.first_name},

            Your appointment with Patient {patientaptdet.pat_id.admin.first_name} is approved.
            Please join the video consultation on {patientaptdet.date_of_appointment} at {patientaptdet.time_of_appointment} using the link below at the scheduled time:
            {jitsi_meet_link}
            ...
            Admin
            """
            recipient_email = patientaptdet.pat_id.admin.email
            try:
                send_mail(subject, message, 'careconnectiiita@gmail.com', [recipient_email],fail_silently=False)
                send_mail(subject, message2, 'careconnectiiita@gmail.com', [doctor_new.email],fail_silently=False)
                patientaptdet.status = status
                patientaptdet.remark = remark
                patientaptdet.save()
                messages.success(request, "Appointment approved, and link shared successfully.")
            except Exception as e:
                messages.error(request, f"Failed to send the email: {e}")
